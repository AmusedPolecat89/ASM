...
pub fn to_canonical_json_bytes<T: Serialize>(value: &T) -> Result<Vec<u8>, AsmError> {
    let value =
        serde_json::to_value(value).map_err(|err| serde_error("asm_web.json_encode", err))?;
    let canonical = canonicalize(value);
    let mut bytes = Vec::new();
    serde_json::to_writer(&mut bytes, &canonical)
        .map_err(|err| serde_error("asm_web.json_write", err))?;
    Ok(bytes)
}

pub fn from_json_slice<T: for<'de> Deserialize<'de>>(data: &[u8]) -> Result<T, AsmError> {
    serde_json::from_slice(data).map_err(|err| serde_error("asm_web.json_read", err))
}
